...
and hh07 = '9' group by deviceid) AS completed_tabs) completed_tabs ";
        ...
        if (isset($searchdata['type']) && $searchdata['type'] == 'c') {
            $sql->whereRaw("(select count(distinct deviceid) from listings where hh01 = l.hh01 and enumcode = l.enumcode  AND (colflag is null or colflag=0 ))!=0
             AND (select count(distinct deviceid) from listings where hh01 = l.hh01 and enumcode = l.enumcode and (hh15!='1' or hh15 is null)  AND (colflag is null or colflag=0))
             = (select count(*) completed_tabs from(select deviceid, max(cast(hh03 as int)) ms from listings where enumcode = l.enumcode and hh01 = l.hh01 and (hh15!='1' or hh15 is null) AND (colflag is null OR colflag = '0')  and hh04 = 9 group by deviceid) AS completed_tabs)");
    ...
    {
        $sql = DB::table('listings');
//        $select = "col_id,tabNo, hh01,hh04,hh07,hh08,hh10,hh05,hh13,hh13a,hh14,hh14a,sysdate, enumcode, _uid,username";
        $select = "*";
        $sql->select(DB::raw($select));
        $sql->where('hh01', '=', $cluster);
        $sql->where('hh13', '=', '1');
